...
class Neo4jHandler:
    ...
    def save_data(self, data):
        with self.driver.session() as session:
            # Ensure the Colombo Stock Exchange node exists
    ...
    def create_cse(tx):
        """Ensure the Colombo Stock Exchange node exists."""
        query = """
        MERGE (cse:StockExchange {name: 'Colombo Stock Exchange'})
        """
        tx.run(query)
    ...
    @staticmethod
    def create_sector_cse_relationship(tx, sector_name):
        """Ensure a sector is listed in the Colombo Stock Exchange."""
        query = """
        MATCH (s:Sector {name: $sector_name})
        MATCH (cse:StockExchange {name: 'Colombo Stock Exchange'})
        MERGE (s)-[:LISTED_IN]->(cse)
        """
        tx.run(query, sector_name=sector_name)
