...
3. Call the **Analysis Agent** to get the technical analysis for the tickers you got from the Io agent. Important : Remember get the tickers first! do not call Analysis agent if you don't have tickers.
...
if you don't have exact tickers you may need to retrieve that using another tool prior to calling this because analysis agent needs exact ticker to do his work
        ...
        2. get the company overview for given ticker
...
Important:  when fetching news you start with news agent and then call io agent for the same query, and provide the aggregated outputs from both agents to the user.
